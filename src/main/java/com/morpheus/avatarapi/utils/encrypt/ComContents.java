...
public class ComContents {
	private static final Logger LOGGER = LoggerFactory.getLogger(ComContents.class);
		
	public final static String SESSION_TOKEN_KEY = "cmas.session.key";
	...
	public static boolean checkAndSaveToken(String tokenKey) {
		...
		// check session...
		if (session.getAttribute(ComContents.SESSION_TOKEN_KEY) == null) {
			throw new RuntimeException("Double Submit Preventer TagLig isn't set. Check JSP.");
		}

		String parameter = request.getParameter(ComContents.PARAMETER_NAME);
		...
		Map<String, String> map = (Map<String, String>) session.getAttribute(ComContents.SESSION_TOKEN_KEY);
